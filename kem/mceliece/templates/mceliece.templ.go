...
const (
	...
	PublicKeySize         = {{.Param.PublicKeySize}}
	PrivateKeySize        = {{.Param.PrivateKeySize}}
	CiphertextSize        = {{.Param.CiphertextSize}}
	SharedKeySize         = 32
	seedSize              = 32
...
func synd(out *[sysT * 2]gf, f *[sysT + 1]gf, L *[sysN]gf, r *[sysN / 8]byte) {
	...
	for i := 0; i < sysN; i++ {
		...
		e := eval(f, L[i])
		eInv := {{.Param.Gf}}.Inv({{.Param.Gf}}.Mul(e, e))
		for j := 0; j < 2*sysT; j++ {
			out[j] = {{.Param.Gf}}.Add(out[j], {{.Param.Gf}}.Mul(eInv, c))
			eInv = {{.Param.Gf}}.Mul(eInv, L[i])
		}
	}
}
...
func minimalPolynomial(out *[sysT]gf, f *[sysT]gf) bool {
	...
	for j := 0; j < sysT; j++ {
		...
		inv := {{.Param.Gf}}.Inv(mat[j][j])
		for c := 0; c <= sysT; c++ {
			mat[c][j] = {{.Param.Gf}}.Mul(mat[c][j], inv)
		}
		...
		for k := 0; k < sysT; k++ {
			if k != j {
				t := mat[j][k]
				for c := 0; c <= sysT; c++ {
					mat[c][k] ^= {{.Param.Gf}}.Mul(mat[c][j], t)
				}
			}
		}
	}
...
func polyMul(out *[sysT]gf, a *[sysT]gf, b *[sysT]gf) {
	...
	for i := 0; i < sysT; i++ {
		for j := 0; j < sysT; j++ {
			product[i+j] ^= {{.Param.Gf}}.Mul(a[i], b[j])
		}
	}
	...
	for i := (sysT - 1) * 2; i >= sysT; i-- {
		...
		product[i-sysT] ^= {{.Param.Gf}}.Mul(product[i], 2)
		{{else if .Is460896}}
		product[i-sysT+10] ^= product[i]
		product[i-sysT+9] ^= product[i]
		product[i-sysT+6] ^= product[i]
